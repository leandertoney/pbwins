...
export default function SponsorBarMobileFixed({ idPrefix = "mobile-bar", position }: SponsorBarMobileFixedProps) {
  ...
  return (
    ...
    >
      <div ref={scrollContainerRef} className="w-full overflow-x-auto px-2 py-1.5 scrollbar-hide">
        <div className="flex gap-2 justify-start">
          {sponsors.map((sponsor, idx) => (
            <div key={`${idPrefix}-${position}-${idx}`} className="flex-shrink-0">
              <SponsorCircle slotIndex={idx} sponsor={sponsor} idPrefix={`${idPrefix}-${position}`} isMobile />
            </div>
          ))}
        </div>
      </div>
