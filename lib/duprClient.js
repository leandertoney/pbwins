...
export async function extractPlayerData(page) {
  ...
  try {
    await page.screenshot({ path: 'dupr-player-debug.png', fullPage: true });
    console.log('✓ Saved screenshot to dupr-player-debug.png');
  } catch (e) {
  ...
  console.log('HTML snippet (first 5000 chars):', htmlSnippet);
  ...
  if (!playerData.fullName) {
    ...
    if (ogTitleMatch) {
      ...
      console.log('Found name from og:title:', playerData.fullName);
    }
  }

  if (!playerData.fullName) {
    // Try page title - but exclude "DUPR"
    const titleMatch = html.match(/<title>([^<]+?)\s*[-–—|]/);
    if (titleMatch && titleMatch[1] !== 'DUPR') {
      playerData.fullName = titleMatch[1].trim();
      console.log('Found name from title:', playerData.fullName);
    }
  }

  // Try more aggressive patterns for player name
  if (!playerData.fullName || playerData.fullName === 'DUPR') {
    // Look for patterns like "Sherwin Torres" in the HTML
    const namePatterns = [
      /<h1[^>]*>([^<]+)<\/h1>/i,
      /"name"\s*:\s*"([^"]+)"/i,
      /"fullName"\s*:\s*"([^"]+)"/i,
    ...
    for (const pattern of namePatterns) {
      ...
      if (match && match[1] && match[1] !== 'DUPR') {
        playerData.fullName = match[1].trim();
        console.log(`Found name with pattern ${pattern}:`, playerData.fullName);
        break;
      }
    }
  }
  ...
  if (!playerData.doublesRating) {
    ...
    const ratingPatterns = [
      /Doubles\s+Rating[^\d]*(\d+\.?\d*)/i,
      /"doublesRating"\s*:\s*(\d+\.?\d*)/i,
      /doubles[^}]*rating[^}]*:\s*(\d+\.?\d*)/i,
    ];

    for (const pattern of ratingPatterns) {
      const match = html.match(pattern);
      if (match) {
        playerData.doublesRating = parseFloat(match[1]);
        console.log(`Found doubles rating with pattern ${pattern}:`, playerData.doublesRating);
        break;
      }
    }
  }
  ...
  if (!playerData.totalWins) {
    ...
    if (winsMatch) {
      ...
      console.log('Found wins:', playerData.totalWins);
    }
  }
